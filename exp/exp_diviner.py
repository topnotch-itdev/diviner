...
from data.data_loader import DatasetETT, DatasetCustom, DatasetGoldCustom
...
class Exp_Diviner(Exp_Basic):
    ...
    def _build_network(self):
        ...
        if len(self.args.load_check_points) != 0:
            net.load_state_dict(torch.load(self.args.load_check_points, map_location=torch.device('cpu')))
    ...
    def test(self, settings):
        ...
        for dates, X, y in test_loader:
            ...
            true_prices = y.detach().cpu().numpy()
            predictions = mse_res['net_out'].detach().cpu().numpy()

            if not self.args.out_scale:
                true_prices = test_dataset.inverse_label_transform(true_prices.reshape(-1, 1)).flatten()
                predictions = test_dataset.inverse_label_transform(predictions.reshape(-1, 1)).flatten()

            MSE_records.append(mse_res['loss'])
            MAE_records.append(mae_res['loss'])

            flattened_dates = [date for sublist in dates for date in sublist]

            all_true_prices.extend(true_prices)
            all_predictions.extend(predictions)
            all_dates.extend(flattened_dates)

        # Save predictions to CSV
        if len(all_predictions) == len(all_true_prices) == len(all_dates):
            df_predictions = pd.DataFrame({
                'date': all_dates,
                'true_price': all_true_prices,
            ...
            print("Length mismatch: Predictions, true prices, and dates must have the same length")
