...
class DatasetGoldCustom(DatasetBasic):
    def __init__(self, args, mode='train'):
        super(DatasetGoldCustom, self).__init__(args, mode)
    ...
    def __getitem__(self, index):
        ...
        if self.model in ['diviner', 'diviner-cg', 'diviner-sc', 'diviner-diff', 'diviner-self masked']:
            ...
            dates = self.dates[x_begin:y_end]

            return dates, data_x, data_y
        else:
            raise KeyError('{} is not supported'.format(self.model))
